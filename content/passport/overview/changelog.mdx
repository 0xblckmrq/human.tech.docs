...
We are also maintaining a separate [Stamp changelog](/passport/building-with-passport/stamps/major-concepts/credential-map-and-weights#stamp-changelog), which tracks updates to our Stamps and credentials within the Passport Stamps product.
...
You can learn more about Passport Embed via our [Passport Embed documentation](/passport/building-with-passport/embed).

## June 18th, 2025

Today we renamed the Passport API to the Stamps API to better align with our new product architecture, which consists of Passport Stamps ([Stamps API](/passport/building-with-passport/stamps/passport-api)), Passport Models ([Models API](/passport/building-with-passport/models)), and [data services](/passport/building-with-passport/data-services).
...
you can learn more about this Stamp via our [Proof of Clean Hands documentation](https://support.passport.xyz/passport-knowledge-base/stamps/how-do-i-add-passport-stamps/the-proof-of-clean-hands-stamp). If a user has verified this Stamp, you will be able to see that they verified this Stamp via the [Stamps API](/passport/building-with-passport/stamps/passport-api) and [smart contracts](/passport/building-with-passport/stamps/smart-contracts).
...
You can learn more about the models in the [Models API documentation](/passport/building-with-passport/models).
...
- We have updated the [Onchain Passport attestation schema](/passport/building-with-passport/stamps/smart-contracts/attestation-schema) to be more developer-friendly.
- We have deprecated the hash that was returned with the decoder contract's `getPassport` method.
- We've introduced a new method, `getScore` (0xdaadd662), which accepts a scorer ID (communityID) and user address, and returns the current score of a user for that scorer. This can be used by partners who are using [custom scores](/passport/building-with-passport/stamps/custom-passport). At this time, there is no way to request Stamp-specific Custom Passport data from onchain. 
