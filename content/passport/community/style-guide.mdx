...
* being clear about the best practices we want to adhere to
...
* lowering barriers to entry by consistently using welcoming and accessible language
...
These personas are best served separately, so that users are not burdened with unnecessary details and developers can easily find the right information to unlock their building.
...
Modular documentation generally follows a hierarchy that first separates information into distinct _user stories_.
...
You should save the image into the `/public` folder and use the relative path to it in the link, rather than linking to an external resource. Any images that require acknowledgements must have them included clearly in the image caption or in text immediately beneath the image. Do not add any images that would break copyright.

### Styles for Users

Users are people that are interested in using Human Passport to demonstrate their identity/credentials. They are not interested in writing code. Users are a very diverse group of people from a range of backgrounds with a variety of experiences and expertise.
