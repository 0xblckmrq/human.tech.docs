...
You can learn more about Passport's available smart contracts and the typical developer flow via our [contract reference page](/passport/building-with-passport/stamps/smart-contracts/contract-reference).
...
The Human Passport smart contracts build on top of [EAS (Ethereum Attestation Service)](https://attest.sh/), using Attestations as the foundational building blocks. You can read the [contract reference](/passport/building-with-passport/stamps/smart-contracts/contract-reference) page for a primer on how the contracts work.
...
**Note** that you can swap out the contract addresses if you want to run an app on a different network. You can check all the deployed contract addresses on the [contract reference page](/passport/building-with-passport/stamps/smart-contracts/contract-reference)

There are some parts of this boilerplate code that might look unfamiliar even if you have been through the other [tutorials](/passport/building-with-passport/stamps/passport-api/quick-start-guide) on this site. This is because there is some specific setup required to use smart contracts on the backend.
