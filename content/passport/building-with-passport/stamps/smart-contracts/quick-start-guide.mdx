...
1. Navigate to the `decoder` contract on a [network](/passport/building-with-passport/stamps/smart-contracts/contract-reference#contract-addresses) supported by onchain Passport. In this example, we'll use the [`decoder` deployed to the Optimism Sepolia network](https://sepolia-optimism.etherscan.io/address/0xe53C60F8069C2f0c3a84F9B3DB5cf56f3100ba56). 
...
Instantiating a contract allows you to interact with a contract deployed on the blockchain as if it were a Javascript object.
...
Explore our [contract reference page](/passport/building-with-passport/stamps/smart-contracts/contract-reference) to find all the details about the various Human Passport contract deployments. 
Then you could try our more advanced [smart contract app tutorial](/passport/building-with-passport/stamps/smart-contracts/integrating-onchain-stamp-data).
