...
**Please note:**<br />This tutorial uses the Stamps API v1. We recommend using [Stamps API v2](/passport/building-with-passport/stamps/passport-api) for new projects.<br /><br />We plan to update this tutorial to use Stamps API v2 shortly.
</Callout>

This tutorial will walk you through verifying an EVM account with both the ETH Activity model (model-based detection) and the [Stamp-based method](/passport/building-with-passport/stamps/passport-api) in your application.

The ETH activity model is one of the [available machine learning models](/passport/building-with-passport/models/available-models) trained on known Sybil and human EVM account data that examines the transaction history for a given Ethereum address and assigns it a trust score. While this tutorial explains how to use the ETH activity model score, you can easily use any of the other available models instead.
...
If you haven't completed the preliminary steps above please refer to our [getting access guide](/passport/building-with-passport/stamps/passport-api/getting-access) first. Once you're done with that, return here and continue with this walkthrough.
...
Replace the contents of `app/page.tsx` with the following boilerplate code (this includes all the very basic logic to render a page and connect a wallet to the app and the logic required to check a Passport - if you need a refresher on the Stamps API logic you can visit our [quick start guide](/passport/building-with-passport/stamps/passport-api/quick-start-guide)):
...
## Adding the Ethereum Activity scorer
